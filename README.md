...
- **Ollama** is the one sidecar not gated behind any Compose profile — it (and its `ollama-pull` companion) always start with a plain `up`, no `--profile` flag needed, so there's a usable local model out of the box. `ollama-pull` is a one-shot init container that pulls `DEFAULT_OLLAMA_MODEL` (default `llama3.2:3b`) into the `ollama` service on every `up` — idempotent, it exits in under a second once the model's already present — so a fresh install never has an empty model list. Change the default model by setting `DEFAULT_OLLAMA_MODEL` in `.env`; using a host Ollama instead (the GPU passthrough case documented on the `ollama` service itself) naturally leaves `ollama-pull` with nothing to do against the container's own volume, since the model ends up in the host's Ollama storage instead.
