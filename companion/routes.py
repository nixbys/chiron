    ...
    def models(request: Request):
        ...
        try:
            ...
            for ep in q.all():
                ...
                # if the SQL filter above were ever loosened. Skipped in
                # single-user/auth-disabled mode, where the caller has no
                # owner identity to compare against and every endpoint
                # (including ones backfilled to a concrete "admin" owner)
                # must remain visible.
