  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false && pkg.name !== 'llama_cpp') {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Chiron.');
    ...
    const _appDepsHtml = (items) => {
      ...
      const parts = [_sectionHeader('Chiron app', 'Run inside the Chiron app itself.')];
      const order = ['System', 'Tools', 'Image', 'LLM', 'Audio', 'Other'];
      for (const cat of order) {
        const catItems = _sortDeps(byCat.get(cat) || [], cat);
        if (!catItems.length) continue;
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast('Pick a GGUF quant first. Chiron will not download the whole GGUF repo without an include pattern.');
          return;
        }
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Chiron capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Chiron keys, choose model directories, and set the default server. Local is this machine.</p>';
