...
function _formatServeCmdPreview(cmd) {
  ...
  if (mlxDeepSeekV4Compat) {
    return [
      '# Chiron runtime compatibility: using sanitized MLX DeepSeek-V4 shim.',
      formatted,
    ].join('\n');
  }
  return formatted;
...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', async (e) => {
      ...
      panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prefix_cache"${sv('prefix_cache',false)?' checked':''} /> Prefix / Radix Cache${_h('vLLM: prefix caching. SGLang: RadixAttention prefix cache; when off Chiron adds --disable-radix-cache.')}</label>`;
